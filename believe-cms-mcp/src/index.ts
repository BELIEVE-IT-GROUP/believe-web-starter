#!/usr/bin/env node
/**
 * believe-cms-mcp — MCP stdio server (the "hands" of the Believe Web Factory).
 *
...
async function loadTools(): Promise<Tool[]> {
  ...
  for (const file of files) {
    ...
    if (!isTool(tool)) {
      // Skip files that do not export a valid tool, but make the noise visible.
      process.stderr.write(`[believe-cms-mcp] skipping ${file}: no valid tool default export\n`);
      continue;
    }
...
async function main(): Promise<void> {
  ...
  const server = new Server(
    { name: "believe-cms-mcp", version: "0.1.0" },
    { capabilities: { tools: {} } },
  );
  ...
  process.stderr.write(
    `[believe-cms-mcp] ready with ${tools.length} tool(s): ` +
...
main().catch((error) => {
  ...
  process.stderr.write(`[believe-cms-mcp] fatal: ${message}\n`);
  process.exit(1);
});
